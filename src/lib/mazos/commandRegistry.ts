...
function rawActions(): Omit<Action,'safetyLevel'>[] { return [
  ...
  // AI first-pass review before human approval (Cobus Greyling, "Eat Your Own AI").
  // Category is Review, NOT Verify: `ocr` has no fail-on-severity flag, so it always exits 0
  // and cannot mechanically gate an iteration (LOOP.md §8). Promote to Verify only behind a
  // wrapper that parses --format json and exits non-zero on blocking findings.
  { id:'review_mazos', label:'AI Review MAZos Diff', description:'open-code-review over the working-tree diff.', category:'Review', enabled:true, dangerLevel:'safe', handler:'command', cwd:PATHS.mazos_ui, command:'ocr', args:['review','--format','json','--audience','agent'], expectedOutput:'JSON review findings with line-level positions', fallbackPrompt:'Run `ocr review` in the MAZos repo and summarize blocking findings only.', timeoutMs: 900000 },
  // Exploratory browser QA. Prompt handler, not a gate: needs an agent driving a browser and
  // returns no mechanical pass/fail. Targets the Next dev server (`npm run dev` -p 3046),
  // which is also Tauri's devUrl. The packaged WebView2 build exposes no CDP endpoint, so
  // agent-browser cannot drive the shipped desktop app — dogfood against dev only.
  { id:'dogfood_mazos', label:'Dogfood MAZos UI', description:'Exploratory browser QA against the running cockpit.', category:'Ops', enabled:true, dangerLevel:'safe', handler:'prompt', expectedOutput:'dogfood-output/report.md with screenshots + repro steps', fallbackPrompt:'Load the agent-browser dogfood skill (`agent-browser skills get dogfood`) and run it against http://127.0.0.1:3046 with `npm run dev` running. Explore every panel systematically, capture a screenshot per issue, and write dogfood-output/report.md with reproduction steps. Report broken states and UX failures, not style opinions.' },
